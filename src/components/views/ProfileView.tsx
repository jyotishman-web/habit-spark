...
import { User, Bell, Trash2, Settings, Share2, ChevronRight, Shield, HelpCircle, LogOut } from 'lucide-react';
    ...
    <div className="min-h-screen pb-24">
      ...
      <div className="px-5 space-y-5 -mt-2">
        <div className="max-w-lg mx-auto space-y-5">
          {/* Profile Card */}
          <div className="bg-card rounded-2xl border border-border/50 shadow-card p-6 text-center">
            <div className="w-20 h-20 gradient-primary rounded-2xl flex items-center justify-center mx-auto shadow-glow">
              <User className="w-10 h-10 text-primary-foreground" />
            </div>
            <h2 className="text-xl font-display font-bold text-foreground mt-4">Habit Builder</h2>
            ...
            <div className="flex items-center justify-center gap-8 mt-6 pt-6 border-t border-border">
              <div className="text-center">
                <p className="text-2xl font-display font-bold text-foreground">{habits.length}</p>
                <p className="text-xs text-muted-foreground font-medium">Habits</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-display font-bold text-foreground">{totalDays}</p>
                <p className="text-xs text-muted-foreground font-medium">Days</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-display font-bold text-foreground">{Math.max(...habits.map(h => h.longestStreak), 0)}</p>
          ...
          {habits.length > 0 && (
            <div className="bg-card rounded-2xl border border-border/50 shadow-card overflow-hidden">
              <div className="p-4 border-b border-border">
                <h3 className="font-display font-semibold text-foreground">Manage Habits</h3>
              </div>
                  ...
                  <div key={habit.id} className="p-4">
                    {showDeleteConfirm === habit.id ? (
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-destructive font-medium">Delete this habit?</span>
          ...
          {/* Sign Out */}
          <button className="w-full flex items-center justify-center gap-2 p-4 text-destructive hover:bg-destructive/5 rounded-xl transition-colors font-medium">
